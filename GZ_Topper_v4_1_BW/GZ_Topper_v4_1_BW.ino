...
void loop() {
  ...
  if (redGI.getStatus(mux) == cityInsertStatus::ON && whiteGI.getStatus(mux) == cityInsertStatus::OFF) {  
    ...
    if (millis() - previousMillis >= 100) {
      ...
      neopixel.fill(neopixel.Color(129, 76, 23), 0, 72);
...
void illuminateMiddleStripWhite() {
  //75, 44, 36
  neopixel.fill(neopixel.Color(129, 76, 23), 72, 72); // starts at light 72 and has a count of 72, which would end on light 144
  neopixel.show();
...
void illuminateBottomStripWhite() {
  //225, 132, 108
  neopixel.fill(neopixel.Color(129, 76, 23), 0, 72); // starts at light 0 and has a count of 72, which would end on light 72
  neopixel.show();
  bottomWhiteStripOn = true;
}
